...
function ProductPreview({ product, onAdd }: { product: Product; onAdd: () => void }) { return <div className="flash-ai-product"><img src={product.image} alt={product.name} /><div><span>{product.brand}</span><Link to={`/product/${product.id}`}>{product.name}<ArrowUpRight size={12} /></Link><b>{formatINR(product.price)} <del>{formatINR(product.mrp)}</del></b></div><button onClick={onAdd} aria-label={`Add ${product.name} to cart`}><Plus size={16} /></button></div>; }
