    ...
    <header className="site-header">
      <div className="header-main shell">
        ...
        <div className="header-actions">
          {user ? <div className="header-account desktop-only"><button className="utility-link utility-link--member" onClick={() => setAccountOpen((current) => !current)}><span className="member-avatar">{user.avatar ? <SafeImage src={user.avatar} alt="" /> : accountInitial}<i aria-label="Active" /></span><span>{user.name.split(" ")[0]}</span></button>{accountOpen && <div className="account-dropdown"><p><span className="account-dropdown__avatar">{user.avatar ? <SafeImage src={user.avatar} alt="" /> : accountInitial}<i /></span><span><b>{user.name}</b><small>{user.email ?? user.phone ?? "Flash account"}</small></span></p><Link to="/account" onClick={() => setAccountOpen(false)}>My Profile</Link><Link to="/orders" onClick={() => setAccountOpen(false)}>My Orders</Link><Link to="/wishlist" onClick={() => setAccountOpen(false)}>Wishlist</Link><Link to="/account#addresses" onClick={() => setAccountOpen(false)}>Saved Addresses</Link><Link to="/flash-club" onClick={() => setAccountOpen(false)}>Flash Club Perks</Link><button onClick={() => { signOut(); setAccountOpen(false); navigate("/"); }}>Sign out</button></div>}</div> : <button className="utility-link desktop-only" onClick={() => setAuthOpen(true)}><UserRound size={20} /><span>Account</span></button>}
          <Link className="utility-link desktop-only" to="/wishlist"><Heart size={20} /><span>Wishlist</span></Link>
          <button className="cart-link" onClick={() => setCartOpen(true)} aria-label={`Cart with ${cartCount} items`}><ShoppingCart size={21} /><motion.span key={cartCount} className="cart-badge" initial={{ scale: 0.6, opacity: 0.5 }} animate={{ scale: [1.35, 0.9, 1], opacity: 1 }} transition={{ type: "spring", stiffness: 420, damping: 20 }}>{cartCount}</motion.span><span className="desktop-only">Cart</span></button>
