...
function decodeGoogleCredential(credential: string): GoogleTokenClaims | null { try { const payload = credential.split(".")[1]; if (!payload) return null; const decoded = atob(payload.replace(/-/g, "+").replace(/_/g, "/")); return JSON.parse(decodeURIComponent(Array.from(decoded).map((character) => `%${character.charCodeAt(0).toString(16).padStart(2, "0")}`).join(""))) as GoogleTokenClaims; } catch { return null; } }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const hydrateGoogleCredential = useCallback((credential: string) => { const claims = decodeGoogleCredential(credential); if (!claims?.sub || !claims.email) return false; signInWithGoogleProfile({ id: claims.sub, name: claims.name?.trim() || claims.email.split("@")[0], email: claims.email, avatar: claims.picture || avatarFor(claims.email) }); return true; }, [signInWithGoogleProfile]);
  const signInWithSocial = useCallback((provider: "Apple") => { const email = "member@icloud.com"; setUser({ id: "apple-member", name: "Apple Member", email, avatar: avatarFor("Apple Member"), flashClub: false, provider }); }, []);
  const signOut = useCallback(() => setUser(null), []);
  const value = useMemo<AuthValue>(() => ({ user, requestOtp, verifyOtp, completePhoneSignup, loginWithPassword, registerAccount, hydrateGoogleCredential, signInWithGoogleProfile, signInWithSocial, signOut }), [user, requestOtp, verifyOtp, completePhoneSignup, loginWithPassword, registerAccount, hydrateGoogleCredential, signInWithGoogleProfile, signInWithSocial, signOut]);
