...
export default function ProductDetail() {
  ...
  return <section className="pdp-page shell">
        ...
        <div className="pdp-thumbnails">
          {activeGallery.map((image, index) => (
            <button
              ...
              key={`${activeVariant?.sku ?? product.sku}-${image}-${index}`}
            >
              <SafeImage src={image} alt={`${product.name} ${activeVariant?.name ?? ""} view ${index + 1}`} />
            </button>
          ))}
        </div>
    ...
    {lightbox && (
      <div className="fixed inset-0 z-50 bg-[#0F1115]/80 backdrop-blur-md flex items-center justify-center p-4" onMouseDown={() => setLightbox(false)}>
        <div className="relative max-w-[85vw] max-h-[85vh] flex flex-col items-center justify-center" onMouseDown={(event) => event.stopPropagation()}>
          <button className="absolute -top-12 right-0 text-white hover:text-[#CCFF00] text-sm font-bold flex items-center gap-1 cursor-pointer" onClick={() => setLightbox(false)}>
            <X size={24} /> Close
          </button>
          <img src={activeGallery[selectedImage] ?? activeGallery[0]} alt={`${product.name} ${activeVariant?.name ?? ""}`} className="max-h-[85vh] max-w-[85vw] object-contain mx-auto rounded-2xl shadow-2xl" />
        </div>
      </div>
    )}
  </section>;
