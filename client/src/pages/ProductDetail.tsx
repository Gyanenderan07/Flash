...
import { formatINR, getDiscount, getProduct, getProductGallery, getProductVariant, products } from "@/data/mockProducts";
...
export default function ProductDetail() {
  ...
  return <section className="pdp-page shell">
    ...
    <div className="pdp-hero">
      <div className="pdp-gallery"><div className="pdp-thumbnails">{activeGallery.map((image, index) => <button className={selectedImage === index ? "is-active" : ""} onClick={() => setSelectedImage(index)} key={`${activeVariant?.sku ?? product.sku}-${image}-${index}`}><img src={image} alt={`${product.name} ${activeVariant?.name ?? ""} view ${index + 1}`} /></button>)}</div><div className={`pdp-media ${zoomed ? "is-zoomed" : ""}`} onMouseEnter={() => setZoomed(true)} onMouseLeave={() => setZoomed(false)}><AnimatePresence mode="wait"><motion.img key={`${activeVariant?.sku ?? product.sku}-${selectedImage}`} initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.01 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} src={activeGallery[selectedImage] ?? activeGallery[0]} alt={`${product.name} in ${activeVariant?.name ?? "default"}`} onClick={() => setLightbox(true)} /></AnimatePresence><button className="zoom-button" onClick={() => setLightbox(true)}><ZoomIn size={18} /> Zoom</button></div></div>
      <div className="pdp-info"><p className="eyebrow">{product.brand} / {product.subcategory}</p><h1>{product.name}</h1><div className="product-meta"><span>SKU: {activeVariant?.sku ?? product.sku}</span><b>Flash verified product</b></div><p className="pdp-description">{product.description}</p><div className="pdp-price"><strong>{formatINR(product.price)}</strong><del>MRP {formatINR(product.mrp)}</del><em>Save {getDiscount(product)}%</em></div><div className="pdp-deal-capsule"><span><Zap size={15} fill="currentColor" /> Flash acceleration</span><b>{getDiscount(product)}% off this product lane</b><small>{product.stock} pieces ready to move. Pick your finish, then move first.</small></div><p className="tax-note">Inclusive of all taxes. Extra cashback routes in at checkout.</p>
        <div className="variant-block"><b>Color: <span>{activeVariant?.name ?? "Standard"}</span></b><div className="swatch-row">{product.colors.map((color) => { const variant = getProductVariant(product, color); return <button key={color} onClick={() => chooseColor(color)} className={selectedColor === color ? "is-selected" : ""} style={{ background: color }} aria-label={`Choose ${variant?.name ?? color}`} title={variant?.name ?? color}><Check size={13} /></button>; })}</div></div>
        {product.sizes && <div className="variant-block"><b>Size: <span>{selectedSize}</span></b><div className="size-row">{product.sizes.map((size) => <button key={size} className={selectedSize === size ? "is-selected" : ""} onClick={() => setSelectedSize(size)}>{size}</button>)}</div></div>}
    ...
    <section className="pdp-details"><div><h2>Built to keep moving.</h2><p>{product.description} Each detail is selected for the hours that do not slow down.</p><ul>{product.highlights.map((highlight) => <li key={highlight}><Check size={15} />{highlight}</li>)}</ul></div><div className="spec-table"><h3>Specifications</h3><dl><div><dt>Brand</dt><dd>{product.brand}</dd></div><div><dt>Category</dt><dd>{product.category}</dd></div><div><dt>Warranty</dt><dd>1 year manufacturer warranty</dd></div><div><dt>Returns</dt><dd>15-day easy return window</dd></div><div><dt>In the box</dt><dd>{product.name}, quick start guide, warranty card</dd></div></dl></div></section>
    <section className="bundle-section"><div className="section-heading"><div><p className="eyebrow">The fast lane</p><h2>Move as a set.</h2></div><button className="black-button" onClick={addBundle}>Build the lane <Plus size={16} /></button></div><div className="bundle-row"><article><img src={activeVariant?.image ?? product.image} alt={product.name} /><div><b>{product.name}</b><span>{formatINR(product.price)}</span></div></article>{pairings.map((item) => <><span className="bundle-plus" key={`${item.id}-plus`}>+</span><article key={item.id}><img src={item.image} alt={item.name} /><div><b>{item.name}</b><span>{formatINR(item.price)}</span></div></article></>)}</div></section>
    ...
    {lightbox && <div className="lightbox-backdrop" onMouseDown={() => setLightbox(false)}><div className="lightbox" onMouseDown={(event) => event.stopPropagation()}><button aria-label="Close gallery" onClick={() => setLightbox(false)}><X size={22} /></button><img src={activeGallery[selectedImage] ?? activeGallery[0]} alt={`${product.name} ${activeVariant?.name ?? ""}`} /></div></div>}
  </section>;
}
